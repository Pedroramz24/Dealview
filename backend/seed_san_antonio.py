...
sample_deals = [
    ...
    },
    {
        ...
        "deal_status": "Under Contract",
        "primary_image_url": "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=800",
        "latitude": 29.4889,
        "longitude": -98.5850,
        "notes": "Strategic location near Medical Center. Long-term tenant. Under contract with 60-day closing period.",
        "contacts": [],
        "documents": [],
        # Transaction Timeline
        "under_contract_date": (datetime.now(timezone.utc) - timedelta(days=10)).strftime("%Y-%m-%d"),
        "earnest_money_due_date": (datetime.now(timezone.utc) - timedelta(days=7)).strftime("%Y-%m-%d"),
        "earnest_money_responsible": "Buyer - ABC Logistics LLC",
        "earnest_money_notes": "$50,000 earnest money deposited with title company",
        "property_info_delivery_date": (datetime.now(timezone.utc) - timedelta(days=3)).strftime("%Y-%m-%d"),
        "property_info_responsible": "Seller - XYZ Properties",
        "property_info_notes": "All property documents delivered including surveys, lease agreements",
        "title_commitment_due_date": (datetime.now(timezone.utc) + timedelta(days=2)).strftime("%Y-%m-%d"),
        "title_commitment_responsible": "Stewart Title Company",
        "title_commitment_notes": "Title review in progress",
        "seller_survey_delivery_date": (datetime.now(timezone.utc) + timedelta(days=7)).strftime("%Y-%m-%d"),
        "seller_survey_responsible": "Seller - XYZ Properties",
        "seller_survey_notes": "Survey ordered from ABC Surveying",
        "feasibility_period_ends_date": (datetime.now(timezone.utc) + timedelta(days=15)).strftime("%Y-%m-%d"),
        "feasibility_period_responsible": "Buyer - ABC Logistics LLC",
        "feasibility_period_notes": "Buyer conducting due diligence on zoning and environmental",
        "buyer_objections_due_date": (datetime.now(timezone.utc) + timedelta(days=20)).strftime("%Y-%m-%d"),
        "buyer_objections_responsible": "Buyer - ABC Logistics LLC",
        "buyer_objections_notes": "Any objections to title or property condition due",
        "seller_response_due_date": (datetime.now(timezone.utc) + timedelta(days=35)).strftime("%Y-%m-%d"),
        "seller_response_responsible": "Seller - XYZ Properties",
        "seller_response_notes": "15 days after buyer objections to respond",
        "closing_date": (datetime.now(timezone.utc) + timedelta(days=50)).strftime("%Y-%m-%d"),
        "closing_responsible": "Stewart Title Company",
        "closing_notes": "Closing scheduled at title company office",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "updated_at": datetime.now(timezone.utc).isoformat(),
        "last_contact": datetime.now(timezone.utc).isoformat(),
        "created_by": "user-1"
